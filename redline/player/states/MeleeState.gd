...
## hang). Cancels: into the next attack or a jump after cancel_time, into an
...
func enter(_previous: StringName) -> void:
	...
	elif attack.sets_air_velocity and combat.take_air_hang():
...
func physics_update(input: PlayerInputFrame, delta: float) -> StringName:
	...
	if attack.is_active_at(t):
		player.combat.melee_query(attack, _hit_ids)
