...
func _count_bolts(room: Room, bolts: Array) -> Callable:
...
func test_short_stops_are_free() -> void:
	...
	var eye := _eye(room)
	var bot := RouteBot.new(get_tree(), room.player)
	var ok: bool = await bot.run([["run", 1200], ["wait", 48]] + EAST_ROUTE)
	await physics_frames(30)
	check(ok, "route failed: %s" % bot.failure)
	check(eye.lock_count == 0 and locked.is_empty(), "a 0.8 s stop must not lock (locks %d)" % eye.lock_count)
...
func test_spawn_past_lost_x_is_gone() -> void:
	var room := await _enter(&"east")
	var eye := _eye(room)
	check(eye.state == CeilingTracker.State.GONE, "a Rook spawned past lost_x is never hunted (state %s)" % eye.state_name())
	var seen := {}
	var cb := _watch(eye, seen)
	var bot := RouteBot.new(get_tree(), room.player)
	...
	check(seen["states"] == ["GONE"], "the eye must never emerge: %s" % str(seen["states"]))
