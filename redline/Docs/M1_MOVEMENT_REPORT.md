...
**Status:** M0 and M1 are implemented. **Development has stopped here for human playtesting (bible §38.13).** Combat (M2) has not been started.
...
All values live in `data/movement/*.tres` (`PlayerMovementConfig`), and camera values in `data/camera/default_camera.tres`. Edit a file in the Godot inspector, save it, and press **F5** in game to reload it. No controller code needs to change.
...
| Tuning without editing controller logic | ✅ | All values in `.tres`; F5 hot reload; F6 presets |
| Stable 60 FPS in lab | ⚠️ Needs real hardware | About 1 ms physics per tick. Only software-rendered FPS was observable here (K-2). |
| Moving for minutes is fun | ⚠️ **This is the gate** | Human playtest required. |

## 6. Known issues
See `KNOWN_ISSUES.md`. The main ones: no human playtest yet (K-1), FPS not verified on real hardware (K-2), possible judder on high-refresh displays (K-3), slides are fully committed (K-5).
...
6. **High refresh:** on a 144 Hz display, compare the current setup against *Project Settings → Physics → Common → Physics Interpolation* (on) and against 120 physics ticks per second.
...
godot --headless --fixed-fps 60 res://tests/TestRunner.tscn        # 29 tests; exit code 1 on failure
