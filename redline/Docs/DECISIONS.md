...
## D-135: First-time Act I pacing grows (~75 s of locked scenes at Normal subtitle speed, ~20-30 s first memory) (FLAG)
...
- Measured first views (Normal subtitle speed): `uc_opening` ≤ 18 s budget, `uc_collector_intro` ≤ 10 s, `relay_arrival` ≤ 13 s, `ll_krail_intro` 12.575 s, `act1_close` 23.17 s with Iko (budget 26 s): about 75 s of locked scenes. Telemetry: `seq_end` carries `first` and `locked`; the report's timeline column subtracts only locking-sequence time.
